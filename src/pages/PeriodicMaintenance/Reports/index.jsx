...
const Reports = () => {
  ...
  const [display, setDisplay] = useState({ maintained: false, nonMaintained: false });
  ...
  useEffect(() => {
    if (periodicReports) {
      const stations = [...new Set(periodicReports.data.map(vehicle => vehicle.lastMaintenance.station))];
      setUniqueStations(stations);
    }
  }, [periodicReports]);

  const maintainedVehicles = periodicReports?.data?.filter((vehicle) => {
    return !(vehicle.isDue || vehicle.isPreAlert || vehicle.isOverdueAlert);
  }) || [];

  const nonMaintainedVehicles = periodicReports?.data?.filter((vehicle) => {
    return vehicle.isDue || vehicle.isPreAlert || vehicle.isOverdueAlert;
  }) || [];
  ...
  const periodicTypeCounts = Object.entries(nonMaintainedByJobType).map(([job, stations]) => {
    const count = Object.values(stations).reduce((sum, vehicles) => sum + vehicles.length, 0);
    return { job, count };
  });
  ...
  const filteredVehicles = selectedJob && selectedStation
      ...
      <div className="dashboard-container">
        <div className="flex justify-center gap-10">
          ...
          <div
            ...
            onClick={() => setDisplay({ maintained: false, nonMaintained: true })}
          >
            <div>
              <p className="text-white text-xl font-semibold">
          ...
          <div className="my-24 grid grid-cols-1 md:grid-cols-2 gap-6">
            {maintainedVehicles.map((vehicle) => (
              <div
                ...
                onClick={() => handleVehicleClick(vehicle.registrationNo, vehicle.lastMaintenance.station)}
              >
                  ...
                  <tbody>
                    {filteredVehicles.map((vehicle) => (
                      ...
                      >
                        ...
                        <td className={vehicle.isDue ? 'text-yellow-500' : vehicle.isPreAlert ? 'text-blue-500' : 'text-red-500'}>
                          {vehicle.isDue ? 'Due' : vehicle.isPreAlert ? 'Pre-Alert' : 'Overdue'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
